...
describe Yacl::Loader::YamlDir do
  ...
  it "can lookup the directory in the passed through configuration if it is not given" do
    ...
    e = Yacl::Loader::YamlDir.new( :configuration => cfg, :parameter  => 'directory' )
    c = e.properties
    c.httpserver.port.must_equal 4321
    c.database.username.must_equal "myusername"
    c.database.subpart.baz.must_equal 'wibble'
  ...
  it "raises an error if no directory was given and it was unable to look it up in a configuration" do
    cfg = Yacl::Properties.new( 'directory' => @yaml_dir )
    lambda { Yacl::Loader::YamlDir.new( :configuration => cfg ).properties }.must_raise Yacl::Loader::YamlDir::Error
  end

  it "raises an error if no directory was given and the value in teh configuration doesn't exist" do
    cfg = Yacl::Properties.new( 'directory' => @yaml_dir )
    lambda { Yacl::Loader::YamlDir.new( :configuration => cfg, :parameter => 'my.directory' ).properties }.must_raise Yacl::Loader::YamlDir::Error
  end
